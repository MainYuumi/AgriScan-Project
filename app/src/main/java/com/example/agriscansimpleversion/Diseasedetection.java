...
public final class Diseasedetection {
  @NonNull
  private final Model model;

  private Diseasedetection(@NonNull Context context, @NonNull Model.Options options) throws
      IOException {
    model = Model.createModel(context, "diseasedetection.tflite", options);
    MetadataExtractor extractor = new MetadataExtractor(model.getData());
  }

  @NonNull
  public static Diseasedetection newInstance(@NonNull Context context) throws IOException {
    return new Diseasedetection(context, (new Model.Options.Builder()).build());
  }

  @NonNull
  public static Diseasedetection newInstance(@NonNull Context context,
      @NonNull Model.Options options) throws IOException {
    return new Diseasedetection(context, options);
  }
